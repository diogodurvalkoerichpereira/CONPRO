...
<table class="table table-sm mt-1 tabelas">
	...
	if($txtbuscar == '' and $cbbuscar == ''){
		$res = $pdo->query("SELECT * from contas order by codigoconta asc LIMIT $limite, $itens_por_pagina");
	}else if($txtbuscar != '' and $cbbuscar == ''){
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from contas where (conta LIKE '$txtbuscar' or descricao LIKE '$txtbuscar') order by conta asc");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from contas where (codigoconta LIKE '$txtbuscar' or conta LIKE '$txtbuscar') and categoria = '$cbbuscar' order by conta asc");
	...
	$dados = $res->fetchAll(PDO::FETCH_ASSOC);
		...
		$res_todos = $pdo->query("SELECT * from contas");
...
echo '
		<tr>
			...
			<td>
				<a href="index.php?acao='.$pagina.'&funcao=editar&id='.$codigoconta.'"><i class="fas fa-edit text-info"></i></a>
				<a href="index.php?acao='.$pagina.'&funcao=excluir&id='.$codigoconta.'"><i class="far fa-trash-alt text-danger"></i></a>
			</td>
		</tr>';
